...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        input1 = self.maxpool(input1)
        input1 = self.layer1(input1)
        feature1 = self.layer2(input1)

        input2 = self.conv1(input2)
        input2 = self.bn1(input2)
        input2 = self.relu(input2)
        input2 = self.maxpool(input2)
        input2 = self.layer1(input2)
        feature2 = self.layer2(input2)

        d_features = torch.abs(feature1 - feature2)
        CA = self.ca(d_features)

        output = self.layer3(CA)
        output = self.layer4(output)
        CA.register_hook((self.save_gradient))
        output_flatten = self.avgpool(output).view(output.size(0), -1)
        result = self.fc(output_flatten)
